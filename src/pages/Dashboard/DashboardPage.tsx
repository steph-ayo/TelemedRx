...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = setupDashboardListener((data) => {
      console.log("📊 Received data from Firebase:", data.length, "items");
      setRequests(data);
      setFilteredRequests(data);
      setLoading(false);
    });
  ...
  const handleStatusChange = async (
    ...
    const result = await updateMedicationStatus(requestId, newStatus);
  ...
  const handleBillingChange = async (requestId: string, billed: boolean) => {
    console.log("💰 Updating billing:", requestId, billed);
    const result = await updateBillingStatus(requestId, billed);
  ...
  const handleDelete = async (requestId: string) => {
    ...
    const result = await deleteMedication(requestId);
